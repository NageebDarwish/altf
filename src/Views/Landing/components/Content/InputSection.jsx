...
const PricingPlans = () => {
  ...
  const handleNavigate = async (planName, cycle) => {
    try {
      ...
      const selectedPlan = plans.find((p) => p.name === planName && p.cycle === cycle);
      const user = /** get user from redux if available */ (function(){ try { return JSON.parse(localStorage.getItem('userData')) || {}; } catch { return {}; } })();
  ...
  const showFree = (isAuthenticated !== "1" && isAuthenticated !== true) && freePlan;
  ...
  if ((isAuthenticated === "1" || isAuthenticated === true) && (isPremium === "1" || isPremium === true)) return "";
  return (
      ...
      <Container className="py-8 md:py-16">
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className={`grid grid-cols-1 gap-4 sm:gap-6 md:gap-8 mt-6 md:mt-8 w-full max-w-4xl mx-auto justify-center items-center ${
              ...
              {showPremium && (
                <div
                  className={`border rounded-2xl bg-heading text-white shadow-md p-4 sm:p-5 md:p-6 flex flex-col items-center text-center relative ${
                    (isAuthenticated === "1" || isAuthenticated === true) && (isPremium !== "1" && isPremium !== true) ? "block" : ""
                  }`}
                >
                  <img
                    src="/Ellipse 54.png"
                    alt=""
                    className={`absolute ${
                      (isAuthenticated === "1" || isAuthenticated === true) && (isPremium !== "1" && isPremium !== true)
                        ? "left-[75px]"
                        : "left-[13px]"
                    } rounded-lg top-0`}
                  />
                      ...
                      {isMonthly ? (
                        ...
                        <>
                          <div className="flex flex-col gap-2">
                            ...
                            <span>
                              ${premiumPlan.price}
                              <span className="text-xs sm:text-sm"> /yearly</span>
                            </span>
                          </div>
                        </>
                      )}
                  ...
                  <div className="bg-white p-4 sm:p-5 md:p-6 w-full rounded-md flex flex-col flex-grow">
                    <ul className="text-heading font-HelveticaNeue space-y-2 sm:space-y-3 md:space-y-4 text-sm sm:text-base md:text-lg text-left flex-grow">
                      ...
                      <li className="flex items-center gap-2 font-bold text-xs sm:text-sm md:text-base">
                        <FaCheck color="#4CD45E" /> Exclusive videos added daily
                      </li>
                      <li className="flex items-center gap-2 font-bold text-xs sm:text-sm md:text-base">
                    ...
                    <div className="mt-auto pt-4 sm:pt-6">
                      <button
                        ...
                        onClick={() => handleNavigate(premiumPlan.name, premiumPlan.cycle)}
